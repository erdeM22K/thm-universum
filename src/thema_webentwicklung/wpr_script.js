...
function allLevelsDone() {
    if (localStorage.getItem('wpr_level1_done') && localStorage.getItem('wpr_level2_done') && localStorage.getItem('wpr_level3_done')) {
        return true;
    } else {
        return false;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    if (allLevelsDone() && !localStorage.getItem('module0_done')) {
        showSkills(1.3);
    ...
    else if (!localStorage.getItem('wpr_visited')) {
        document.querySelector("#overlay").style.display = "block";
        gsap.to("#overlay", {
            duration: 1,
            delay: 1,
